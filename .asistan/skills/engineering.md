...
- before finishing an implementation step, write the commit message for
  your change to `.asistan-commit-message` at the repo root:
  `<type>: description` per `CONTRIBUTING.md` above, describing what you
  actually changed — not a placeholder. This is what gets committed and
  used as the PR title. It's removed before staging, so it never lands in
  the commit itself. Steps that do not change code (`analyze`, `review`)
  must not write it.
