...
/// State 06 · konum izni yok (docs/design/app-states.md): shown instead of
/// the map on first launch before the permission decision and whenever
/// permission is denied — [LocationService.resolveInitialCenter]'s
/// `permissionDenied` reason, never its other fallback reasons (disabled
/// service, timeout), which keep the map's existing silent-fallback banner.
/// Ships with only the primary action per the issue #121 approval: the
/// secondary "haritada elle mahalle seç" action has no implementation
/// owner, and the privacy sub-line is unverified against hosting
/// access-log retention (gap 4) — both dropped from this pass rather than
/// guessed.
class LocationPermissionState extends StatelessWidget {
  const LocationPermissionState({super.key, required this.onRequestPermission});
