...
issue #236 renames this concept in the product, not on the wire: what the user sees is the cat's **profile photo** (a circular avatar on cat detail), and the wide cover-photo surface is gone. the endpoint path, the `is_cover` field on `GET /v1/cats/{cat_id}/media`, and the `cats.primary_photo_id` column deliberately keep their names — renaming them would mean a migration and a breaking response change for the 0.3 client still in the field, for no behavioral gain (issue #236's own compatibility section). read `cover` in this document as "the cat's one canonical image"; the client's own naming says profile photo. a video can never be selected: only entries from the cat's photo archive are eligible, which the client enforces by not offering the action on a video entry and the server by requiring a `media_id` that belongs to the cat's archive.
...
`POST /v1/media` (issue #70; extended by issue #153's update-attached photo and video flow) is standalone media upload — independent of cat creation (a cat's own required initial photo is instead embedded directly in `POST /v1/cats`, above, and stays image-only). Validation, ownership resolution, and `Idempotency-Key` handling are identical to `POST /v1/cats`'s photo for images (same shared pipeline, see [[backend]]). This endpoint additionally accepts a narrow, pass-through-validated video: a recognized mp4/mov container (`ftyp` major_brand `isom`/`mp42`/`mp41`/`M4V `/`qt  `) no longer than 30 seconds, per the file's own `moov`/`mvhd` duration — never a client-supplied value. No transcoding is performed for this first version; the uploaded bytes are stored as-is. An oversized upload answers `413`, a recognized-but-too-long video answers `400 {"error":"video too long"}`, and anything that's neither a decodable jpeg/png nor a recognized video container answers `400 {"error":"malformed file"}`. Since issue #241, the validated image or video also goes through pre-publication content moderation before it's stored — a video is reduced to a 3-frame contact sheet and classified through the same vision path as a photo (see [[backend]]); a rejection answers `422 {"error":"content rejected"}`, and a moderation-provider or frame-extraction failure answers the generic `500`. Neither ever stores the object or creates a media row. `POST /v1/cats/{cat_id}/updates` may carry an optional `media_id` referencing a row the same authenticated account just uploaded here; the update write resolves that id, rejects an unknown-or-not-owned media row as `404 {"error":"media not found"}`, and surfaces the attached media back as `photo_url` plus `media_content_type` on the created update and later `GET .../updates` reads — the client tells a video apart from a photo by checking whether `media_content_type` starts with `video/`.
