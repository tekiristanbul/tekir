...
**0.2 note (contract only, not yet implemented):** issue #100's simplified help contract (`docs/product/alerts.md`) supersedes the category vocabulary as a user-facing concept — `yardıma ihtiyacı var` becomes a flag on the ordinary update write, combinable with statuses, with an optional note and no categories — and this doc's category paragraphs describe the implemented 0.1 wire contract until the migration lands in #101. the target 0.2 shape, the mixed 0.1/0.2 client-compatibility constraints, and the open migration questions blocking #101 are recorded in that product doc, not here.
