...
re-marking while a help state is already active (product-owner decision on the #100 contract's open question, implemented in #101): the write succeeds and the 72h window restarts from the newer mark, but followers are **not** notified again — the notification worker's claim query decides suppression against the mark's own `created_at` (see [[db]]), so a delayed dispatch reaches the same verdict a prompt one would.
