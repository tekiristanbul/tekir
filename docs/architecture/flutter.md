...
- **network**: `dio` behind a single `ApiClient`; a `DeviceInterceptor` (issue #32, implemented) attaches `X-Device-Token: <device_token>` when a stored device identity is available, and `Authorization: Bearer` when logged in — two separate headers, since `Authorization` can't unambiguously carry both schemes at once (see [[api]]). the interceptor reads the cached identity synchronously and never blocks requests while registration is still in flight, so public read routes remain usable from the first frame.
...
- every `dio` request goes through the `DeviceInterceptor`: `X-Device-Token` is attached when a cached identity is available (synchronous, non-blocking); `Authorization: Bearer` when present is attached too. the token is only sent to the configured tekir api origin.
