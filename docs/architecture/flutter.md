...
- **network**: `dio` behind a single `ApiClient`; a `DeviceInterceptor` (issue #32, implemented) attaches `X-Device-Token: <device_token>` when a stored device identity is available. the interceptor reads the cached identity synchronously and never blocks requests while registration is still in flight, so public read routes remain usable from the first frame. authenticated account flows will need a separate auth interceptor later so `Authorization: Bearer` stays an unambiguous single-purpose header (see [[api]]).
...
- **push**: `firebase_messaging`; a future push-token refresh flow will call `PUT /v1/devices/me`, but that endpoint is not implemented in issue #32.
...
- on startup, a `deviceIdentityProvider` checks secure storage for a `device_id` and `device_token`; if absent, calls `POST /v1/devices` using a separate non-intercepted `Dio` instance (so registration never waits on itself) and persists both only after a valid server response. concurrent first-use calls share a single registration attempt. if initialization fails, read-only browsing continues and a later `resetFailedInit()` call allows a controlled retry without a tight loop. implemented in issue #32.
- every `dio` request goes through the `DeviceInterceptor`: `X-Device-Token` is attached when a cached identity is available (synchronous, non-blocking). the token is only sent to the configured tekir api origin. a separate auth interceptor can attach `Authorization: Bearer` later once account flows are implemented.
