...
- **network**: `dio` behind a single `ApiClient`; an interceptor attaches `Authorization: Device <device_token>` (and `Authorization: Bearer <access_token>` when logged in) to every request, matching [[api]]'s auth model.
...
- every `dio` request goes through the interceptor: the `Device` token is always attached; `Bearer access_token`, when present, is attached too.
