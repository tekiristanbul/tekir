...
- **notification worker**: same repo, separate binary/process, polling the `notification_outbox` table (see [[db]]) and dispatching fcm pushes to followers. a full message broker (kafka/rabbitmq) is unwarranted at this scale. the fan-out follows the insert-before-push order documented in [[db]], which gives **at-most-once** delivery: no follower is ever double-pushed, but a crash between recording and actually sending a push loses it silently rather than retrying. that small loss window is accepted for mvp, not hidden behind "idempotent" language that implies stronger guarantees than this provides.
