...
- in mvp, only a newly-created active needs-help update for a followed cat triggers a push notification. this trigger rule is unchanged by the 0.2 simplified help contract ([[alerts]], issue #100; implemented by #101); notification copy is already category-free and stays as-is, and the push data payload's `category` key was dropped in #101.
- a help mark made while the cat's help state is already active does not notify followers again (product-owner decision on issue #101) — the state simply continues with a restarted 72-hour window.
