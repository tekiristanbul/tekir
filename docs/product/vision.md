...
- one-sentence definition: an app where a user can see cats nearby, add photos or videos to a cat, and post structured status updates (`seen`, `fed`, `water_provided`) with an optional free-text comment; an update requires at least one structured status, so a comment alone is not valid. traits such as "friendly" or "playful" belong to the cat's profile ([[cats]]), not to a status update. a user can also post a needs-help update, which expires automatically after exactly 72 hours and remains in the cat's history without active emphasis once expired.
