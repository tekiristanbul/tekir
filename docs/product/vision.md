...
- one-sentence definition: an app where a user can see cats nearby, add photos or videos to a cat, and post status updates with labels (for example: friendly, playful). the user can also add a comment, and can add updates that expire after a set time (for example 24h or 3h).
