...
- a cat waiting for help pulses too, on its own and at its own slower rhythm, wherever it is drawn as its own face. pulse is reserved for exactly these two things and nothing else in the product uses it. the ring at rest is part of the marker itself, at every resolution — so a cat needing help is still marked when the platform asks for reduced motion and nothing pulses at all.
