...
- a needs-help alert ([[alerts]]) is part of the same update history. in 0.1 (implemented) it is a distinct subtype that must show its fixed category clearly. in 0.2 (issue #100, pr #103 product-owner decision, contract in [[alerts]]) the category vocabulary is superseded: `yardıma ihtiyacı var` becomes one of the options in the update screen, selectable alongside `gördüm`, `mama verdim`, and `su verdim` in a single update that appears as one timeline event; the reason lives in the reporter's optional free-text note, and there are no help subcategories.
