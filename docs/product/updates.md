...
- a needs-help alert ([[alerts]]) is part of the same update history. in 0.1 (implemented) it is a distinct subtype that must show its fixed category clearly. in 0.2 (issue #100, contract in [[alerts]]) the category vocabulary is superseded: help is a single `yardıma ihtiyacı var` state and the reason lives in the reporter's optional free-text note. whether 0.2 keeps help as a separate record or makes it a flag an update can carry together with statuses is an unresolved decision recorded in [[alerts]].
