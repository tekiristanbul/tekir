...
3. **re-marking while a help state is already active: no re-notification; the window restarts.** the update is recorded normally and the 72-hour window follows the newest mark, but followers are not notified again while the state is already active. suppression is decided once, at the mark's own creation time, and recorded with it (#105) — delayed notification dispatch reaches the same verdict, and removing the earlier mark afterwards never turns the suppressed one into a notification.
...
7. **presentation (issue #287, 0.5).** on the map a cat needing help carries a ring in the help colour plus a badge shape, and on its detail screen the same ring surrounds its profile photo and pulses — the two surfaces say it the same way, so arriving from a tapped help pin reads as one continuous thing. pulse is used for exactly two things in the product, help and map selection, and nothing else. the ring is decoration: the state, when it was raised and when it ends are always present in text beside it, so reduced motion holds the ring still rather than removing it, and help is never signalled by colour alone. help keeps its own colour family and never becomes a shade of the primary accent; what 0.5 changed is the treatment — a soft block with dark help text, the reporter's note set as a quotation — not the semantics. there is still nothing to press: expiry is the only clearing mechanism (decisions 1 and 2).
