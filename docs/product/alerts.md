...
- creating a needs-help update requires authenticated bearer identity. ordinary text-only updates may be posted using anonymous device identity without account login; needs-help is the explicit authenticated exception to that anonymous text-only update rule recorded in [[trust]].
